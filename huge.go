...
// or arena memory. Each allocation is backed by its own slice.
//
...
// from 64 KB to 16 MB; buffers larger than 16 MB bypass the pool.
//
// Huge is designed for buffers larger than 64 KB: large WAL segments,
// compaction buffers, bulk I/O pages.
//
//	h := slabix.NewHuge()
...
type hugeEntry struct {
	buf   []byte
	class int // size class index, -1 = no pool
}

const (
	hugeMinClass    = 16 // 64 KB = 1 << 16
	hugeMaxClass    = 24 // 16 MB = 1 << 24
...
// Alloc allocates a byte slice of exactly size bytes. The slice is
// zeroed. The caller must pass the same slice to [Huge.Free] when done.
...
// buffer must not be used after Free. If pool reuse is enabled, the
// buffer may be returned to an internal pool for future allocations.
...
// sizeClass returns the pool index for a given aligned size, or -1 if
// the size doesn't fit any pool class.
...
// sliceKey returns a stable map key derived from the slice's backing
// array pointer.
